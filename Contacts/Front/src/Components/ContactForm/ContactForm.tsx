...
function ContactForm() {
  ...
  useEffect(()=>{
    if(p.id){
      axios.get(`http://localhost:5141/contacts/get/${p.id}`).then(res => {
            ...
            const resg = toJalaali(date.getFullYear(),Number(month),Number(day))
             
            console.log(date.getFullYear())
            const converteddate = `${resg.jy}-${resg.jm}-${resg.jd}`
  ...
  const submitHandle = (data:FieldValues) =>{
    const isuniq = contacts.filter(c => c.phon_number == data.phon_number)
    if(isuniq.length != 0 && !iseditmode){
      alert("شماره همراه قبلا ذخیره شده است")
      return
    }
     const resj = toGregorian(Number(data.birthday.year),Number(data.birthday.month),Number(data.birthday.day))
    const dateObject = new Date(`${resj.gy}-${resj.gm}-${resj.gd}T00:00:00.000Z`);
      ...
      <form onSubmit={handleSubmit(submitHandle)} className="w-full flex flex-col gap-3 p-4">
        ...
        {...register("birthday" , {required:"مقادیر را پر کنید"}) } type="date" placeholder="تاریخ تولد"></input> */}
         <Controller
          control={control}
          name="birthday"
          rules={{ required: {value : true , message: "لطفا تاریخ تولد را انتخاب کنید"} ,}} //optional
          render={({
            ...
            <>
              <DatePicker
                value={value || ""}
                containerClassName="p-4 bg-amber-50 text-gray-600 rounded-md"
                onChange={(date) => {
